export const translations = {
  es: {
    header: {
      home: "Inicio",
      services: "Servicios",
      about: "Nosotros",
      contact: "Contáctanos",
      news: "Noticias",
    },
    hero: {
      title: "Get it easy, get it safe!",
      subtitle: "¿Qué te lo impide? Nosotros lo solucionamos.",
      cta: "Contáctanos",
    },
    pillars: {
      comercializadora: "Comercializadora",
      logistica: "Logística",
      aduanal: "Despacho Aduanal",
    },
    services: {
      title: "Servicios que ofrecemos",
      items: [
        { title: "Importaciones y Exportaciones", desc: "Tránsito de mercancía nacional e internacional." },
        { title: "Asesoría Legal Aduanal", desc: "Asesoría especializada para el despacho aduanal." },
        { title: "Cuaderno ATA", desc: "Gestión de carnés ATA para mercancía temporal." },
        { title: "Trámites BANJERCITO", desc: "Gestión de trámites ante BANJERCITO." },
        { title: "Certificaciones NOMs de Seguridad", desc: "Certificaciones de NOMs de seguridad para tu mercancía." },
        { title: "Cumplimiento de NOMs de Etiquetado", desc: "Gestión del cumplimiento de NOMs de etiquetado." },
        { title: "Padrón Sectorial", desc: "Textil, Calzado y Siderúrgico." },
        { title: "Almacén / Depósito Fiscal", desc: "Gestión y administración de almacenes y depósitos fiscales para tu mercancía." },
      ],
      logisticsTitle: "Servicios Logísticos",
      logistics: [
        { title: "Flete Terrestre", desc: "Cobertura nacional con tiempos de entrega óptimos y rastreo en tiempo real." },
        { title: "Logística Internacional", desc: "Soluciones multimodales para conectar tu negocio con mercados globales." },
        { title: "Flete Marítimo", desc: "Coordinación integral de envíos marítimos desde el origen hasta la entrega." },
        { title: "Flete Aéreo", desc: "Envíos express para mercancía urgente o de alto valor al mejor precio." },
      ],
    },
    coverage: {
      title: "Cobertura",
      desc: "Operamos en más de 4 estados de la república mexicana, destacando Quintana Roo, Yucatán, CDMX y Nuevo Laredo.",
    },
    news: {
      title: "Noticias",
      viewAll: "Ver todas las noticias",
    },
    indicators: {
      title: "Tipo de Cambio (Dólar DOF)",
      startDate: "Fecha Inicio",
      endDate: "Fecha Fin",
      button: "Consultar",
      modalTitle: "Histórico de Tipo de Cambio",
      modalDesc: "Resultados oficiales obtenidos del Diario Oficial de la Federación (DOF) para solventar obligaciones denominadas en moneda extranjera.",
      dateCol: "Fecha",
      valueCol: "Valor (MXN)",
      loading: "Consultando tipo de cambio...",
      noData: "No se encontraron datos para el periodo seleccionado.",
      error: "Ocurrió un error al obtener la información desde el DOF.",
      validationError: "Por favor, seleccione un rango de fechas válido.",
      todayRate: "Dólar hoy:",
    },
    contact: {
      title: "¿Tienes alguna duda?",
      desc: "Nuestro equipo de atención a clientes estará disponible para dar seguimiento a tus necesidades.",
      cta: "Contáctanos",
    },
  },
  en: {
    header: {
      home: "Home",
      services: "Services",
      about: "About Us",
      contact: "Contact Us",
      news: "News",
    },
    hero: {
      title: "Get it easy, get it safe!",
      subtitle: "What's stopping you? We solve it.",
      cta: "Contact Us",
    },
    pillars: {
      comercializadora: "Trading Company",
      logistica: "Logistics",
      aduanal: "Customs Clearance",
    },
    services: {
      title: "Our Services",
      items: [
        { title: "Imports & Exports", desc: "National and international merchandise transit." },
        { title: "Legal Customs Advisory", desc: "Specialized advice for customs clearance." },
        { title: "ATA Carnet", desc: "ATA carnet management for temporary goods." },
        { title: "BANJERCITO Procedures", desc: "Processing procedures before BANJERCITO." },
        { title: "Safety NOM Certifications", desc: "Safety NOM certifications for your merchandise." },
        { title: "Labeling NOM Compliance", desc: "Labeling NOM compliance management." },
        { title: "Sectoral Registry", desc: "Textile, Footwear and Steel sectors." },
        { title: "Warehouse / Fiscal Deposit", desc: "Management and administration of warehouses and fiscal deposits for your merchandise." },
      ],
      logisticsTitle: "Logistics Services",
      logistics: [
        { title: "Land Freight", desc: "National coverage with optimal delivery times and real-time tracking." },
        { title: "International Logistics", desc: "Multimodal solutions to connect your business with global markets." },
        { title: "Maritime Freight", desc: "End-to-end coordination of maritime shipments from origin to delivery." },
        { title: "Air Freight", desc: "Express shipments for urgent or high-value merchandise at the best price." },
      ],
    },
    coverage: {
      title: "Coverage",
      desc: "We operate in more than 4 states of Mexico, including Quintana Roo, Yucatán, CDMX and Nuevo Laredo.",
    },
    news: {
      title: "News",
      viewAll: "View all news",
    },
    indicators: {
      title: "Exchange Rate (USD DOF)",
      startDate: "Start Date",
      endDate: "End Date",
      button: "Query",
      modalTitle: "Historical Exchange Rate",
      modalDesc: "Official exchange rate values retrieved from the Official Gazette of the Federation (DOF) to settle obligations denominated in foreign currency.",
      dateCol: "Date",
      valueCol: "Value (MXN)",
      loading: "Fetching exchange rate...",
      noData: "No exchange rate data was found for the selected period.",
      error: "An error occurred while fetching information from the DOF.",
      validationError: "Please select a valid date range.",
      todayRate: "USD today:",
    },
    contact: {
      title: "Have a question?",
      desc: "Our customer service team will be available to follow up on your needs.",
      cta: "Contact Us",
    },
  },
};